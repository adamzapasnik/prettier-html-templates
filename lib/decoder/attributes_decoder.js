...
const isInElement = (parts) =>
  parts.some((part) => {
    if (Array.isArray(part)) return isInElement(part);
    return typeof part === 'string' && part.match(/eex[a|c]?\d+/);
  });
...
const decodeInAttributeNames = (parts, expressionMap, shouldIndent = false) => {
  ...
  for (const [index, part] of parts.entries()) {
    ...
    if (Array.isArray(part)) {
      decodedParts.push(decodeInAttributeNames(part, expressionMap, shouldIndent));
...
const decodeInAttributeValues = (parts, expressionMap) => {
  ...
  for (const part of parts) {
    if (Array.isArray(part)) {
      decodedParts.push(decodeInAttributeValues(part, expressionMap));
