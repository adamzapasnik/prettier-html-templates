...
console.log('re', replaceEndOfLineWith)
        ...
        return [
          ...
          group(
            mapDoc(embeddedAttributeValueDoc, (doc) =>
              // TODO: this needs to take expressions into consideration I guess
              // Or needs to be done earlier?
...
function genericPrint(path, options, print) {
  ...
  switch (node.type) {
    ...
    case "attribute": {
      ...
      // TODO: this could break in the expression?
      // TODO: this could break in non string node.value huh!
      const value = unescapeQuoteEntities(node.value);
      const singleQuoteCount = countChars(value, "'");
      const doubleQuoteCount = countChars(value, '"');
      const quote = singleQuoteCount < doubleQuoteCount ? "'" : '"';

      console.log('whathere', node.value)
      console.log('wtf',
      replaceEndOfLineWith(
        quote === '"'
          ? value.replace(/"/g, "&quot;")
      ...
      return [
        ...
        "=",
        quote,
        // TODO: this can be probably broken too=.=
        ...replaceEndOfLineWith(
          quote === '"'
            ? value.replace(/"/g, "&quot;")
            : value.replace(/'/g, "&apos;"),
...
function printClosingTagStart(node, options) {
  // if (node.lastChild && node.lastChild.type === "expression") {
  //   return ""
  // }
  return node.lastChild &&
    needsToBorrowParentClosingTagStartMarker(node.lastChild) //&& node.lastChild.type !== "expression"
    ? ""
    : [
...
function needsToBorrowNextOpeningTagStartMarker(node) {
  ...
  if (node.next && node.next.type === "expression") return false
  return (
    node.next &&
...
function needsToBorrowPrevClosingTagEndMarker(node) {
   ...
   */
   ...
   if (node.prev && node.prev.type === "expression") return false
...
function needsToBorrowLastChildClosingTagEndMarker(node) {
   ...
   */
   ...
   if (node.lastChild && node.lastChild.type === "expression") return false
