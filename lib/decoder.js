...
const decodeExpressions = (expressionMap) => {
  ...
  return (doc) => {
    ...
    const parts = Array.isArray(doc) ? doc : doc.parts ? [...doc.parts] : [doc];
    const decodedParts = [];
    ...
    if (isSelfClosingAfterOpenTag2(parts)) {
      const { removeWhitespace, decodedParts: newDecodedParts } = decodeInSelfClosingAfterOpenTag(parts, expressionMap);
      opts.removeWhitespace = removeWhitespace;
      decodedParts.push(...newDecodedParts);
    } else if (isSelfClosingInText(parts)) {
    ...
    return Array.isArray(doc)
      ? decodedParts
      : doc.parts
      ? Object.assign({}, doc, { parts: decodedParts })
      : decodedParts[0];
  };
};
