...
var _a = prettier.doc.builders, breakParent = _a.breakParent, group = _a.group, line = _a.line;
...
export var decodeExpressions = function (expressionMap) {
    ...
    return function (doc) {
        ...
        if (isInTableOrHead(parts)) {
            ...
            var partlyDecodedDoc = __assign(__assign({}, doc), { parts: decodeInTableOrHead(parts, expressionMap) });
            // deals with the rest of encoded
            return decodeExpressions(expressionMap)(partlyDecodedDoc);
        }
        if (isSelfClosingInText(parts)) {
            var _a = decodeSelfClosingInText(parts, expressionMap), removeWhitespace = _a.removeWhitespace, newDecodedParts = _a.decodedParts;
            opts.removeWhitespace = removeWhitespace;
            decodedParts.push.apply(decodedParts, newDecodedParts);
        }
        else if (isInElement(parts)) {
            decodedParts.push.apply(decodedParts, decodeInAttributes(parts, expressionMap));
        }
        else {
