...
const { ParseSourceSpan } = require('angular-html-parser/lib/compiler/src/parse_util');
...
const PREPROCESS_PIPELINE = [
  ...
  addIsSpaceSensitive,
  mergeSimpleElementIntoText, // aa<span>emmmmmmmmmmmmmmmmmmmmmmmmmmme</span>b
];
...
function removeIgnorableFirstLf(ast /*, options */) {
  return ast.map((node) => {
    if (
      ...
      return node.clone({
        children: text.value.length === 1 ? rest : [text.clone({ value: text.value.slice(1) }), ...rest],
      });
    }
    return node;
  });
...
function mergeIeConditonalStartEndCommentIntoElementOpeningTag(ast /*, options */) {
  /**
   *     <!--[if ...]><!--><target><!--<![endif]-->
   */
  const isTarget = (node) =>
    ...
    if (node.children) {
      ...
      if (shouldMergeResults.some(Boolean)) {
        ...
        for (let i = 0; i < node.children.length; i++) {
          ...
          const newChild = child.type === 'text' ? child : child.clone({ type: 'text', value: getValue(child) });

          if (newChildren.length === 0 || getLast(newChildren).type !== 'text') {
...
function extractWhitespaces(ast /*, options*/) {
  ...
  return ast.map((node) => {
    ...
    if (
      node.children.length === 0 ||
      (node.children.length === 1 && node.children[0].type === 'text' && htmlTrim(node.children[0].value).length === 0)
    ) {
      return node.clone({
        children: [],
    ...
    node.type !== 'element'
      ...
      : Object.assign(node, {
          hasHtmComponentClosingTag:
            node.endSourceSpan &&
            /^<\s*\/\s*\/\s*>$/.test(
              options.originalText.slice(node.endSourceSpan.start.offset, node.endSourceSpan.end.offset)
            ),
        })
  );
}
...
function addCssDisplay(ast, options) {
  return ast.map((node) => Object.assign(node, { cssDisplay: getNodeCssStyleDisplay(node, options) }));
}
        ...
        .map((child, index, children) => ({
          ...
          isLeadingSpaceSensitive:
            index === 0
              ...
              : children[index - 1].isTrailingSpaceSensitive && child.isLeadingSpaceSensitive,
          isTrailingSpaceSensitive:
            index === children.length - 1
              ? child.isTrailingSpaceSensitive
              : children[index + 1].isLeadingSpaceSensitive && child.isTrailingSpaceSensitive,
        })),
    });
  });
}
